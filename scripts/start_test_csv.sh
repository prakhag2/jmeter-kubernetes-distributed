...
# i.e. jmx file name has to be the same as directory name.
# After execution, test script jmx file may be deleted from the pod itself but not locally.

# Get namesapce
namespace="$1"
[ -n "$namespace" ] || read -p 'Enter namespace for load test setup ' namespace
...
kubectl cp "${jmx_dir}/${jmx_dir}.jmx" -n "$namespace" "$master_pod":/
...
kubectl exec -ti -n "$namespace" "$master_pod" -- /jmeter/load_test "/${jmx_dir}.jmx"
